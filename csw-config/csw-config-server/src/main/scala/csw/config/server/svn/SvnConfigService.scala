...
class SvnConfigService(settings: Settings, actorRuntime: ActorRuntime, svnRepo: SvnRepo, annexFileService: AnnexFileService)
  ...
  private def createFile(path: Path, configData: ConfigData, annex: Boolean = false, comment: String): Future[ConfigId] = {
    ...
    def createAnnex(): Future[ConfigId] =
      async {
  ...
  override def update(path: Path, configData: ConfigData, comment: String): Future[ConfigId] = {
    ...
    def updateAnnex(): Future[ConfigId] =
      async {
  ...
  override def getByTime(path: Path, time: Instant): Future[Option[ConfigData]] = {
    ...
    def getHist: Future[Option[ConfigFileRevision]] =
      async {
  ...
  private def get(path: Path, configId: Option[ConfigId] = None): Future[Option[ConfigData]] =
    async {
  ...
  private def getNormal(path: Path, revision: SVNRevision): Future[Option[ConfigData]] =
    async {
  ...
  private def getAnnex(path: Path, revision: SVNRevision): Future[Option[ConfigData]] =
    async {
  ...
  override def setActiveVersion(path: Path, id: ConfigId, comment: String = ""): Future[Unit] =
    async {
  ...
  override def resetActiveVersion(path: Path, comment: String): Future[Unit] =
    async {
      if (!await(exists(path))) {
        throw FileNotFound(path)
      }
  ...
  override def getActive(path: Path): Future[Option[ConfigData]] = {
    ...
    def getActiveById(configId: ConfigId): Future[Option[ConfigData]] =
      async {
  ...
  private def pathStatus(path: Path, id: Option[ConfigId] = None): Future[PathStatus] =
    async {
  ...
  private def getCurrentVersion(path: Path): Future[Option[ConfigId]] =
    async {
      await(pathStatus(path)) match {
        case PathStatus.NormalSize =>
  ...
  private def hist(path: Path, from: Instant, to: Instant, maxResults: Int): Future[List[ConfigFileRevision]] =
    async {
