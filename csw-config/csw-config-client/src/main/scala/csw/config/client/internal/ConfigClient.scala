...
private[config] class ConfigClient(
  ...
  private implicit class HttpRequestExt(val httpRequest: HttpRequest) {
    ...
    def withBearerToken: HttpRequest =
      bearerTokenHeader match {
  ...
  override def getActive(path: jnio.Path): Future[Option[ConfigData]] =
    async {
      await(get(await(activeConfig(path))))
    }

  override def getLatest(path: jnio.Path): Future[Option[ConfigData]] =
    async {
  ...
  override def exists(path: jnio.Path, id: Option[ConfigId]): Future[Boolean] =
    async {
  ...
  override def getActiveByTime(path: jnio.Path, time: Instant): Future[Option[ConfigData]] =
    async {
      await(get(await(activeConfig(path)).withQuery(Query("date" -> time.toString))))
    }

  override def getActiveVersion(path: jnio.Path): Future[Option[ConfigId]] =
    async {
  ...
  private def handleActiveConfig(path: jnio.Path, query: Query): Future[Unit] =
    async {
  ...
  private def get(uri: Uri): Future[Option[ConfigData]] =
    async {
      val request = HttpRequest(uri = uri)
      maskTokenAndLog(request)
      val response = await(Http().singleRequest(request))
