...
public class JConfigAdminApiTest extends JUnitSuite {
    ...
    public void testCreateAndRetrieveFile__DEOPSCSW_42_DEOPSCSW_88_DEOPSCSW_138_DEOPSCSW_103_DEOPSCSW_48() throws ExecutionException, InterruptedException {
        ...
        Assert.assertEquals(configData.orElseThrow().toJStringF(system).get(), configValue1);
    }
    ...
    public void testUpdateExistingFile__DEOPSCSW_88_DEOPSCSW_138_DEOPSCSW_103_DEOPSCSW_49() throws ExecutionException, InterruptedException {
        ...
        Assert.assertEquals(configDataUpdated.orElseThrow().toJStringF(system).get(), configValue2);
    }
    ...
    public void testSpecificVersionRetrieval__DEOPSCSW_88_DEOPSCSW_138_DEOPSCSW_71_DEOPSCSW_103_DEOPSCSW_70() throws ExecutionException, InterruptedException {
        ...
        Assert.assertEquals(configService.getLatest(path).get().orElseThrow().toJStringF(system).get(), configValue3);

        Assert.assertEquals(configService.getById(path, configId).get().orElseThrow().toJStringF(system).get(), configValue2);
    }
    ...
    public void testRetrieveVersionBasedOnDate__DEOPSCSW_88_DEOPSCSW_138_DEOPSCSW_103() throws ExecutionException, InterruptedException {
        ...
        Assert.assertEquals(configService.getLatest(path).get().orElseThrow().toJStringF(system).get(), configValue3);
        Assert.assertEquals(configService.getByTime(path, instant).get().orElseThrow().toJStringF(system).get(), configValue2);
    }
    ...
    public void testGetSetAndResetActiveConfigFile__DEOPSCSW_77_DEOPSCSW_139_DEOPSCSW_78_DEOPSCSW_88_DEOPSCSW_138_DEOPSCSW_103_DEOPSCSW_79_DEOPSCSW_70() throws ExecutionException, InterruptedException {
        ...
        configService.update(path, ConfigData.fromString(configValue3), "Updated config to assembly").get();
        Assert.assertEquals(configService.getActive(path).get().orElseThrow().toJStringF(system).get(), configValue1);

        configService.setActiveVersion(path, configIdUpdate1, "setting active version").get();
        Assert.assertEquals(configService.getActive(path).get().orElseThrow().toJStringF(system).get(), configValue2);

        configService.resetActiveVersion(path, "resetting active version of file").get();
        Assert.assertEquals(configService.getActive(path).get().orElseThrow().toJStringF(system).get(), configValue3);
    ...
    public void testActiveVersionBasedOnTime__DEOPSCSW_88_DEOPSCSW_138_DEOPSCSW_103_DEOPSCSW_140() throws ExecutionException, InterruptedException {
        ...
        Assert.assertEquals(configService.getActiveByTime(file, tHeadRevision).get().orElseThrow().toJStringF(system).get(), configValue1);
        Assert.assertEquals(configService.getActiveByTime(file, tActiveRevision1).get().orElseThrow().toJStringF(system).get(), configValue2);
        Assert.assertEquals(configService.getActiveByTime(file, Instant.now()).get().orElseThrow().toJStringF(system).get(), configValue3);
    }
