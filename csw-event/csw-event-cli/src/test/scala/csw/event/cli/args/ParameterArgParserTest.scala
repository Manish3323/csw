...
class ParameterArgParserTest extends AnyFunSuite with Matchers {
  ...
  test("should able to parse valid single param string when key name, type, units & values provided | DEOPSCSW-436") {
  ...
  test("should able to parse valid string params | DEOPSCSW-436") {
    ParameterArgParser.parse("k1:s=['a, b','cd']|k2:s=['x,y','z']") shouldBe
    Set(StringKey.make("k1").set("a, b", "cd"), StringKey.make("k2").set("x,y", "z"))
  }

  test("should able to parse valid string params with single quote and comma | DEOPSCSW-436") {
  ...
  test("should able to parse all valid space separated multi param string | DEOPSCSW-436") {
  ...
  test("should throw exception when values are not provided for some of the param | DEOPSCSW-436") {
    intercept[RuntimeException](ParameterArgParser.parse("k:i=1|k2:s"))
  }

  test("should throw exception when unsupported key type provided | DEOPSCSW-436") {
    intercept[RuntimeException](ParameterArgParser.parse("k:x:volt=1v"))
  }

  test("should throw exception when invalid unit provided | DEOPSCSW-436") {
