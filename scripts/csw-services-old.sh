...
#    csw-services-old.sh start --initRepo     :to start event service, cluster seed application and register the config and event service
#
...
#    csw-services-old.sh start    :to start event service, cluster seed application and register the config and event service
#
...
#    csw-services-old.sh start --interfaceName eth0   :to start event service, cluster seed application and register the config and event service
#
#   In case you do not want to start all the services, then specify supported arguments to start
#    csw-services-old.sh start --seed 5552 --config 5555   :this starts cluster seed on port 5552 and config server on port 5555 (does not start redis)
#
#    csw-services-old.sh stop     :to stop event service and unregister the services from the location service
#
...
function start_seed() {
    ...
    if [[ -x "$location_script" ]]; then
        ...
        nohup ./csw-location-server --clusterPort ${seed_port} -J-DCLUSTER_SEEDS=${seeds} -J-Dcsw-location-server.http-port=${location_http_port} &
    ...
    start)
        ...
        if [[ ${INTERFACE_NAME} == "" ]]; then
            ...
            if [[ ${IP} == "" ]]; then
                ...
                seeds="${IP}:${seed_port}"
                echo "[INFO] Using CLUSTER_SEEDS=$seeds"
